...
for i = 1:num_categories
    ...
    for j=1:num_samples
        if(j <= length(train_examples))
            ...
            fprintf(fid,'<img src="%s" width=%.10f height=%.10f>', ['thumbnails/' categories{i} '_' name '.jpg'], width, height);
    ...
    for j=1:num_samples
        if(j <= length(false_positives))
            ...
            fprintf(fid,'<img src="%s" width=%.10f height=%.10f>', ['thumbnails/' false_positive_labels{j} '_' name '.jpg'], width, height);
